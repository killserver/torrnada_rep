...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['act']))
{
    ...
    elseif ($_GET['act'] == "delete" && $_GET['targetid']>0)
    {
            sql_query("DELETE FROM friends WHERE userid = ".intval($_GET['targetid'])." AND friendid = " . sqlesc($CURUSER['id']));
            sql_query("DELETE FROM friends WHERE friendid = ".intval($_GET['targetid'])." AND userid = " . sqlesc($CURUSER['id']));
            $dt = sqlesc(get_date_time());
            ...
            sql_query("INSERT INTO messages (sender, receiver, added, msg, subject) VALUES (0, ".intval($_GET['targetid']).", ".$dt.", ".$msg.", ".$subj.")") or sqlerr(__FILE__, __LINE__);
            header("Refresh: 2; url=" . $DEFAULTBASEURL . "/friends.php");
            stderr("Успешно", "Пользователь удален из друзей.");
    }
...
if(mysql_num_rows($res) == 0)
    ...
    $body1 = "<a href=\"userdetails.php?id=" . $friend['id'] . "\"><b>" . get_user_class_color($friend["class"], $friend['name']) . "</b></a>" .
		...
		$body2 = "<br /><a href=\"friends.php?id=".$userid."&act=delete&type=friend&targetid=" . $friend['id'] . "\">".$tracker_lang['delete']."</a>" .
...
else
{
	...
	while ($block = mysql_fetch_array($res))
	{
		if ($i % 6 == 0)
			$blocks .= "<tr>";
    	$blocks .= "<td style='border: none; padding: 4px; spacing: 0px;'>[<font class=\"small\"><a href=\"friends.php?id=".$userid."&act=delete&type=block&targetid=" .
