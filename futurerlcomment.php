...
{
$futurerlid = intval($_POST["tid"]);
if (!is_valid_id($futurerlid))
stderr("Ошибка", "Неверный ID $futurerlid.");
$res = sql_query("SELECT id FROM futurerls WHERE id = ".$futurerlid) or sqlerr(__FILE__,__LINE__);
...
sql_query("INSERT INTO comments (user, trailer, added, text, ori_text, ip) VALUES (" . $CURUSER["id"] . ", ".$futurerlid.", '" . get_date_time() . "', " . sqlesc($text) . "," . sqlesc($text) . "," . sqlesc(getip()) . ")") or sqlerr(__FILE__,__LINE__);
$newid = mysql_insert_id();
sql_query("UPDATE futurerls SET comments = comments + 1 WHERE id = ".$futurerlid) or sqlerr(__FILE__,__LINE__);
...
$res = sql_query("SELECT c.*, o.id FROM comments AS c LEFT JOIN futurerls AS o ON c.trailer = o.id WHERE c.id=".$commentid) or sqlerr(__FILE__,__LINE__);
...
stderr("Удаление комментария", "Если вы уверены нажмите\n<a href=\"futurerlcomment.php?action=delete&cid=".$commentid."&sure=1" . ($referer ? "&returnto=" . urlencode($referer) : "") ."\">тут</a>.");
}
$res = sql_query("SELECT trailer FROM comments WHERE id=".$commentid)  or sqlerr(__FILE__,__LINE__);
$arr = mysql_fetch_array($res);
if ($arr)
...
sql_query("UPDATE futurerls SET comments = comments - 1 WHERE id = ".$futurerlid);
...
elseif ($action == "quote") 
{
        ...
        echo ("<tr><td class=\"colhead\" align=\"left\">Редактировать комментарий к \"" . htmlspecialchars_uni($name) . "\"</td><tr>\n");
