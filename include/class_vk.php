...
class Vkontakte
{
	...
	public function file_send($url, $file) {
		$ch = curl_init($url);
     ...
     */
    public function postToPublic($publicID, $text, $fullServerPathToImage=null, $tags = array())
    {
	if(!empty($fullServerPathToImage)) {
	        ...
	        $output = array();
		$output = $this->file_send($uploadURL, $fullServerPathToImage);
